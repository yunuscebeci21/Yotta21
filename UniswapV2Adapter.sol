...
contract UniswapV2Adapter is IUniswapV2Adapter {
  ...
  function addLiquidity() external override returns (bool) {
    ...
    uint256 _ttffAmount = (_ttff.balanceOf(address(this))).mul(_ttffPrice).div(
      10**18
    );
  ...
  function removeLiquidity(uint256 _percent) external override returns (bool) {
    ...
    uint256 _wethAmountMin = (_percentLiquidity.mul(_reserveWeth).div(10**18))
      .sub((_percentLiquidity.mul(_reserveWeth).mul(5).div(10**19)));
    uint256 _ttffAmountMin = (_percentLiquidity.mul(_reserveTtf).div(10**18))
